...
export async function load(params) {
	...
	try {
		...
		post = await collection.find({'slug': slug}).toArray();
	} catch (error) {
		console.log(error);
	}	
	...
	statistics = await gscCollection.find({'slug': slug}).sort({ date: 1 }).toArray();
	} catch (error) {
		console.log(error);
	}

	// Get mod history from mod_history collection
	var modHistory = []
	try {
		const modHistoryCollection = database.collection('mod_history');
		modHistory = await modHistoryCollection
			.find({'slug': slug})
			.sort({ date: -1 })
			.toArray();
	} catch (error) {
	...
	await client.close();
  return {post: structuredClone(post[0]), statistics: structuredClone(statistics), modHistory: structuredClone(modHistory)};
};
...
export const actions = {
	default: async (event) => {
		...
		console.log(`getting stats from ${startDate} to ${today}`);
		...
		console.log("Closing connection and returning success");
		await client.close();
